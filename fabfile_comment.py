...
# print(os.path.dirname(os.path.abspat(__file__)))
...
env.key_filename = 'zzini.pem' #동일 폴더 위치
...
def _setup():
    ...
    # 프로젝트 별로 필요한 패키지를 설치하여 상호 프로젝트 간 
    # 영향을 받지 않게 만드는 방식(가상환경)
    


# 7-2. apt(ubunt상에서) 패키지를 업데이트 여부 확인 및 업데이트
def _init_apt():
...
def update():
    ...
    _git_update()
    # 8-2 가상환경 필요한 패키지 설치 (1회만수행)
    _virtualenv_update()
    # 8-3 아파치에 가상 호스트 세팅 (1회만수행)
    _virtualhost_make()
    # 8-4 아파치가 프로젝트르 access할 수 있게 권한 처리 (1회만수행)
    #       but 신규 파일에 대한 검토 필요 -> 그냥 매번 수행
    _grant_apache()
    # 8-5 서버 재가동(아파치)
    _restart_apache()
